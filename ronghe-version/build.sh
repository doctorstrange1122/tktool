...
# ============================================
# 融合工具 APK 构建脚本
# 包名: com.tktool.ronghe，独立签名，与正式版共存
# ============================================
set -e
...
KEYSTORE="$OUT_DIR/ronghe_v2.keystore"
KEYSTORE_PASS="ronghe2026"
KEY_ALIAS="ronghe_v2"
KEY_PASS="ronghe2026"

echo "========================================"
echo "  融合工具 APK 构建"
echo "  包名: com.tktool.ronghe"
echo "========================================"
...
javac -source 1.8 -target 1.8 \
    ...
    "$SRC"/java/com/tktool/ronghe/*.java "$OUT_DIR"/gen/com/tktool/ronghe/R.java 2>/dev/null || \
javac -source 1.8 -target 1.8 \
    -cp "$ANDROID_JAR:$ZXING_JAR" \
    -d "$OUT_DIR/java_classes" \
    "$SRC"/java/com/tktool/ronghe/*.java
echo "       编译完成"
...
if [ ! -f "$KEYSTORE" ]; then
    keytool -genkey -v \
        ...
        -dname "CN=RongHe, OU=Dev, O=RongHe, L=Shenzhen, ST=Guangdong, C=CN"
fi
...
echo ""
echo "========================================"
echo "  APK: $OUT_DIR/ronghe_tool.apk"
echo "  大小: $(ls -la "$OUT_DIR/ronghe_tool.apk" | awk '{print $5}') bytes"
echo "========================================"
